...
- tags on `main` only (`v0.5.0` is current). `Cargo.toml` workspace version tracks the latest released tag.
...
- **homebrew formula and npm package install the binary only**: the dist-generated formula in the `hoffresearch/homebrew-urna` tap (and the `@urna/cli` npm package, and `cargo install`/`binstall`) does not lay down the embedder payload, so a brew-installed `urna` reports exit 4 from `urna doctor` until the user also runs the one-liner (or copies `python/forge/` into the data dir by hand). fixing this means a custom formula, deferred until the tap sees real use.
...
- `docs/CHANGELOG`: 0.1.0 through 0.5.0 and the unreleased deltas, with measured numbers.
