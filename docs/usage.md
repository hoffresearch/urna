...
the release workflows assume external state that a fresh org does not have. as of 2026-09-26 (repo secrets of `hoffresearch/urna`: `CARGO_REGISTRY_TOKEN` and `NPM_TOKEN` set, `HOMEBREW_TAP_TOKEN` missing):

1. **homebrew tap**: the public repo `hoffresearch/homebrew-urna` exists with an empty `Formula/` directory (created 2026-09-26); the `publish-homebrew-formula` job in `release.yml` checks it out and pushes `Formula/urna.rb` to it. it still needs a fine-grained token with contents read and write on that repo only, stored as the `HOMEBREW_TAP_TOKEN` secret of `hoffresearch/urna`; github has no api that mints one, so it is created by hand in the account settings. without it the job fails and the rest of the release stands.
